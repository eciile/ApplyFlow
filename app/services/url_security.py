...
async def resolve_hostname(
    ...
    if not addresses:
        raise UnsafeUrlError("The URL hostname did not resolve to an IP address.")
...
def ensure_global_ip(address_value: str) -> None:
    ...
    try:
        ...
        raise UnsafeUrlError("The hostname resolved to an invalid IP address.") from exc

    if not address.is_global:
        raise UnsafeUrlError("The URL resolves to a non-public IP address.")
